...
public filePreview:any
  ...
  ngOnInit() {
    console.log(this.parentSubject);
    
    if (this.parentSubject) {
      this.parentSubject.subscribe(async(res) => {
        console.log(res);
          if (res) {
            if (this.type == 3) {
    ...
    const upsertResponse = fileItem => {
      ...
      this.zone.run(() => {
        ...
        if (existingId > -1) {
          ...
          if (this.multiple == "false" && this.responses && this.responses.length > 0) {
            const single = this.responses[this.responses.length - 1]
            this.responses = []
            console.log(this.responses);
